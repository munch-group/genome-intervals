...
3. Replaces all instances of "genome-intervals" with the module name
4. Renames the src/genome_intervals directory to src/{modulename}
...
def main():
    ...
    # if module_name == "genome_intervals":
    #     print("Module name is already 'genome_intervals', no changes needed.")
    #     return
    ...
    for file_path in files_to_process:
        if find_and_replace_in_file(file_path, "genome-intervals", repo_name, args.dry_run):
            files_changed += 1
            status = "Would update" if args.dry_run else "✓"
            print(f"  {status} {file_path}")
        if find_and_replace_in_file(file_path, "genome_intervals", module_name, args.dry_run):
            files_changed += 1
            status = "Would update" if args.dry_run else "✓"
            print(f"  {status} {file_path}")
    ...
    if rename_directory("genome_intervals", module_name, args.dry_run):
